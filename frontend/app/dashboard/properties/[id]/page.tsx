...
export default function PropertyDetailPage() {
  ...
  const fetchExpenses = async (propertyId: string | string[]) => {
    try {
      ...
      const expensesData = Array.isArray(res.data) ? res.data : [];
      const total = expensesData.reduce(
        (sum: number, exp: any) => sum + parseFloat(exp.amount || 0), 0
      );
      setPropertyExpenses(total);
    } catch (err) {
            ...
            <div className="mb-4 p-3 bg-purple-50 rounded-xl">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 accent-[#581c87]"
                />
                <span className="text-lg font-bold text-[#581c87] w-12 text-right">
                  {commissionRate}%
                </span>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-gray-700">
                  -{Math.round(monthlyRevenue * commissionRate / 100).toLocaleString()} MAD
                </span>
              </div>
